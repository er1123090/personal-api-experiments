...
def process_with_gpt(
    ...
    for _, row in tqdm.tqdm(df.iterrows(), total=len(df), desc="Processing Examples"):
        ...
        # [필터링 로직 수정] pref_type에 따라 필요한 키가 있는지 확인
        if pref_type == "easy":
        ...
        # [Modified] 호출 시 pref_type과 pref_group_path 전달
...
if __name__ == "__main__":
    ...
    parser.add_argument("--query_path", type=str, default="/data/minseo/personal-tool/conv_api/experiments2/temp_queries.json")
    parser.add_argument("--pref_list_path", type=str, default="/data/minseo/personal-tool/conv_api/experiments2/pref_list.json")
    
    # [New Argument] Preference Group File Path
    parser.add_argument("--pref_group_path", type=str, default="/data/minseo/personal-tool/conv_api/experiments2/vanillaLLM/pref_group.json", help="Path to pref_group.json for hard")

    parser.add_argument(
        "--context_type", 
        type=str, 
        choices=["diag-only", "apilist-only", "diag-apilist"], 
        required=True,
        help="Choose what context to include: 'diag_only', 'api_only', or 'both'."
    )
    
    # [Modified choices] Added 'hard'
    parser.add_argument(
        "--pref_type", 
        type=str, 
    ...
    process_with_gpt(
        ...
        pref_group_path=args.pref_group_path, # Passed here
